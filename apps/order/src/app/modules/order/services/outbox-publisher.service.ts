...
export class OutboxPublisherService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly outbox: OutboxEventRepository) {}

  async onModuleInit(): Promise<void> {
    const { BROKERS, CLIENT_ID } = CONFIGURATION.KAFKA_CONFIG;
    if (!BROKERS?.length) {
      this.logger.warn('Kafka brokers empty; outbox publisher will not run');
      return;
    }
  ...
  private async publishBatchSafe(): Promise<void> {
    try {
      ...
      this.logger.warn(`Outbox poll error: ${(e as Error).message}`);
    }
  }
