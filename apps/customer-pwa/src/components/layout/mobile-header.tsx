...
export function MobileHeader() {
  ...
  const displayName = session?.restaurantName ?? 'QRTable';
  const displayTable = session?.tableName ?? '—';
  const displaySession = session?.sessionId ? `${session.sessionId.slice(0, 8)}…` : 'Chưa có phiên';
    ...
    <header className="sticky top-0 z-20 border-b border-border bg-background/95 backdrop-blur">
      <div className="mx-auto flex w-full max-w-screen-sm flex-col gap-4 px-4 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Phiên đặt món QR</p>
            <h1 className="mt-1 text-lg font-semibold tracking-tight text-foreground">{displayName}</h1>
            <p className="mt-1 text-xs text-muted-foreground">Mã phiên: {displaySession}</p>
          </div>

          <div className="rounded-lg border border-border bg-card px-3 py-2 text-right">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Bàn hiện tại</p>
            <p className="text-sm font-semibold text-card-foreground">{displayTable}</p>
          </div>
        </div>

        <nav aria-label="Điều hướng luồng khách hàng" className="grid grid-cols-4 gap-2">
          {NAV_ITEMS.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
