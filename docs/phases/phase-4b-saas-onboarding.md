...
- Saga evidence update on 2026-05-31: opt-in SaaS PostgreSQL integration passed the onboarding success path plus rollback before and after subscription assignment, and opt-in live Payment TCP integration passed creation of exactly one Payment-owned `tenant_payment_settings` row. The tests use UUID-valid owner fixtures because `tenants.owner_id` is a PostgreSQL `uuid` column. This evidence strengthens the mini-saga claim while keeping Authorizer/Keycloak and User-Access as contract-doubled boundaries in the current automated proof.
