# Plan thảo luận Phase 8C — Defense Deck QRTable

> **Dành cho agent:** Đây là plan thảo luận và handoff, chưa phải yêu cầu dựng deck ngay. Không tạo `.pptx`, không sửa LaTeX report, không thêm citation, không chỉnh `references.bib` cho đến khi người dùng chốt hướng triển khai. Mục tiêu trước mắt là tiếp tục trao đổi, tinh chỉnh mạch trình bày, phạm vi chứng minh, và chiến lược demo cho khóa luận QRTable.

## 1. Mục tiêu của plan

Plan này là handoff đã chốt để một agent trong session mới có thể triển khai defense deck QRTable, nhưng không bị lan man vào toàn bộ codebase.

Mục tiêu chính:

- Tạo cơ sở triển khai deck PowerPoint chính thức cho buổi bảo vệ, không còn ở giai đoạn hỏi lại narrative nền.
...
- Chốt deck khoảng **30 slide chính** thay vì 17-18 slide, vì 17-18 slide quá ít để trình bày đầy đủ bài toán, kiến trúc, cơ chế, và bằng chứng.
- Chuẩn bị nền để sau này có thể dựng PowerPoint (`.pptx`) làm bản chính và PDF làm bản dự phòng.
...
- Số slide chính đã chốt: **30 slide chính** cho phần trình bày 20-25 phút; chỉ dao động nhẹ khi implementation buộc phải xử lý overflow, không được làm mất cụm cơ chế trọng yếu.
...
- Visual direction đã chốt: **A — Product Dark**, lấy cảm hứng trực tiếp từ landing page của Management App.
...
Dependency còn lại trước bản final:

- Logo chính thức của Trường Đại học Công nghệ Thông tin. Nếu chưa có khi bắt đầu triển khai, dùng structured placeholder `GLOBAL_SCHOOL_LOGO`; không được xem là final evidence/asset.
- Evidence thật cho screenshot, database state, log/trace hoặc test output. Có thể dựng placeholder có cấu trúc trước, nhưng bản final phải thay bằng asset thật hoặc ghi rõ giới hạn.
...
## 4. Skill nên dùng trong session mới

Agent mới nên dùng skill theo nhu cầu, không dùng hình thức.
...
- Dùng `$writing-plans` nếu chuyển plan thảo luận này thành implementation plan thật.
...
Target hiện tại: khoảng **30 slide chính** cho phần trình bày 20-25 phút. Demo tách riêng.
...
1. **Số slide chính**
   - Chốt 30 slide chính cho 20-25 phút.
...
6. **Cách bắt đầu session triển khai**
   - Agent mới phải đọc ba file bắt buộc, dùng CodeGraph trước, rồi tạo implementation plan riêng cho deck source/PPTX/PDF.
   - Sau implementation plan, agent có thể bắt đầu dựng deck theo plan này nếu không phát hiện blocker.
...
Người dùng đã chọn **A — Product Dark** từ visual comparison ngày 2026-06-15.

Deck cần giữ nhận diện mạnh của landing page QRTable:

- nền tối sâu;
- accent cyan và emerald;
- grid/dot/diagonal texture nhẹ;
- radial glow có kiểm soát;
- card tối bán trong suốt, viền mảnh;
- motif QR, luồng dữ liệu và đường nối;
- typography sans hiện đại kết hợp mono cho nhãn kỹ thuật;
- screenshot sản phẩm đặt trong device/frame hoặc evidence panel có chủ đích.

Không sao chép nguyên landing page theo từng section. Slide phải chuyển hóa ngôn ngữ sản phẩm thành ngôn ngữ trình bày kỹ thuật.
...
- Heading/body chính: **Plus Jakarta Sans**.
- Nhãn kỹ thuật, số bước, topic/event, slide metadata: **IBM Plex Mono**.
...
- Nhãn nhỏ `QRTable · Khóa luận tốt nghiệp`.
- Slide number kín đáo.
- Logo Trường Đại học Công nghệ Thông tin ở góc trên bên phải hoặc footer cố định.
- Cover có thể dùng logo lớn hơn; các slide nội dung dùng logo nhỏ để không cạnh tranh với luận điểm.
...
Deck Product Dark không có nghĩa mọi slide dùng cùng một grid card. Nên có khoảng 7-9 layout tái sử dụng:

1. Cover có ảnh F&B full-bleed, dark overlay, QR/glow motif.
...
Trong giai đoạn dựng storyboard hoặc khi chưa có asset thật, deck được phép dùng placeholder. Tuy nhiên placeholder phải là **hợp đồng nội dung có cấu trúc (structured content contract)**, không phải một ô trống chỉ ghi “đặt ảnh ở đây”.

Mỗi asset nên được quản lý trong một registry từ mã nguồn deck với tối thiểu các trường:

| Trường            | Ý nghĩa                                                                          |
| ----------------- | -------------------------------------------------------------------------------- |
| `id`              | Mã ổn định, ví dụ `SLIDE_23_ORDER_SAGA`                                          |
| `type`            | `logo`, `diagram`, `screenshot`, `database-state`, `log`, `test-output`, `photo` |
| `path`            | Đường dẫn asset thật; có thể để trống khi chưa có                                |
| `status`          | `placeholder`, `ready`, `verified`                                               |
...
| `source`          | Code/test/doc/runtime hoặc file nguồn dự kiến                                    |
| `aspectRatio`     | Tỉ lệ khung mong muốn để tránh thay asset làm vỡ layout                          |
...
const assets = {
  schoolLogo: {
    ...
    source: 'Asset chính thức do người dùng cung cấp hoặc nguồn trường đã kiểm chứng',
  ...
  orderConfirmSaga: {
    ...
    source: 'CodeGraph + Order/Catalog code + Saga tests; render từ Mermaid/SVG',
...
Khi `path` chưa tồn tại hoặc `status === 'placeholder'`, generator phải render một placeholder theo đúng theme Product Dark, gồm:

- asset ID;
- loại asset;
- mục đích cần chứng minh;
- danh sách nội dung bắt buộc;
- nguồn dự kiến;
- trạng thái hiện tại.
...
1. Chuẩn bị file thật theo đúng yêu cầu của registry.
2. Đặt file vào thư mục asset ổn định.
...
- Diagram nên giữ source có thể review như `.mmd`, `.svg` hoặc source code tương đương; PPTX dùng artifact đã render.
...
- Placeholder không được tính là evidence thật và không được xuất hiện trong bản deck final.
...
3. Kiểm tra asset registry: không còn asset bắt buộc ở trạng thái `placeholder` trước bản final.
...
## 14. Prompt đề xuất cho agent session mới

Copy prompt dưới đây vào session mới để triển khai deck:
...
Mục tiêu của session này là triển khai defense deck cho khóa luận tốt nghiệp QRTable theo plan đã chốt. Deck gồm PPTX bản chính, PDF bản dự phòng, 30 slide chính cho 20-25 phút trình bày. Demo không nằm trong 20-25 phút.
...
- Visual direction: A — Product Dark.
- Theme lấy cảm hứng trực tiếp từ landing page Management App: nền zinc tối, cyan #22D3EE, emerald #34D399, grid/glow/card tối, motif QR và dòng chảy dữ liệu.
...
- Mỗi placeholder phải ghi asset ID, loại, mục đích cần chứng minh, nội dung bắt buộc, nguồn dự kiến, tỉ lệ khung, caption và trạng thái `placeholder/ready/verified`.
...
- $writing-plans nếu cần chuyển plan thảo luận thành implementation plan.
...
- Dựng source deck sinh PPTX theo outline 30 slide đã chốt trong `thesis-defense-deck-methodology-plan.md`.
...
- Khi triển khai visual, giữ hướng Product Dark nhưng kiểm tra deck không biến thành product pitch: giảm CTA, pricing pattern, glow quá mạnh và card density.
```

## 15. Trạng thái chuyển sang thực thi

Plan đã đủ điều kiện để chuyển sang session triển khai:
...
- Appendix đã chốt là có.
- Visual direction đã chốt: Product Dark.
- SaaS onboarding bị loại khỏi deck chính và appendix.
- Order Confirm Saga là Saga chính để đào sâu.

Các việc vẫn cần xử lý trong quá trình triển khai, không phải blocker để bắt đầu:
...
- Thay placeholder screenshot, database state, log/trace và test output bằng asset thật nếu muốn dùng như bằng chứng final.
- Export PDF từ PPTX đã QA; không tạo deck PDF riêng bằng LaTeX.

Agent triển khai phải tạo implementation plan riêng, rồi mới tạo/sửa deck.
