...
| Event-driven architecture                 | `docs/technical-architecture.md` mục 7; `docs/implementation_plan.md`; Kafka-related tests                                                                                           | Cao                  | Phải trình bày là selective event-driven architecture, không phải "mọi thứ đều async". Hiện có sáu approved Kafka topics.               |
...
| Deployment/observability                  | `docker-compose.provider.yaml`; `docs/phases/phase-5-7-finalization.md`; package scripts                                                                                             | Trung bình - thấp    | Chỉ nên viết như phương án thiết kế/vận hành demo có giới hạn; khi claim trace/dashboard/deploy thực tế thì cần bổ sung bằng chứng sau. |
...
- Payment domain: cash, VietQR/SePay, webhook settlement, payment outbox, refund baseline, payment-to-order finalization.
...
- Auth/RBAC: Keycloak/JWT cho staff/admin; session/HMAC token cho customer; permission matrix với 66 permissions.
...
| Notification Service     | Proposal dự kiến có service độc lập.                                         | Có thể trình bày notification như một phần trong kiến trúc hoàn thiện nếu cần, nhưng khi nói implementation evidence phải kiểm tra lại code.                                            | Hiện `apps/notification` chưa tồn tại; nếu Phase 4C được implement sau, cập nhật lại evidence và diagram. |
...
| Kafka topics             | Proposal nhắc `order.created`, `kitchen.order-ready` và event flow rộng.     | Dùng topic registry hiện tại: `order.confirmed`, `order.status_changed`, `payment.completed`, `payment.refunded`, `kitchen.sla_warning`, `tenant.created`.                              | UI events dùng BFF Direct/Redis hints. Không invent Kafka topics trong báo cáo.                           |
