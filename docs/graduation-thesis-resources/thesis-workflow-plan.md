...
> Cập nhật gần nhất: 2026-06-15 (chốt handoff triển khai Phase 8C defense deck; 30 slide chính, demo riêng 5–7 phút, có appendix).
...
| Phase 8C | Defense deck                               | Deck bảo vệ 20–25 phút, speaker notes, appendix phản biện, bản PDF dự phòng và demo fallback                                         | Đã chốt handoff triển khai `thesis-defense-deck-methodology-plan.md`; deck artifact sẽ được tạo trong session triển khai |
...
7. Session triển khai có thể bắt đầu dựng deck sau khi tạo implementation plan riêng. Logo UIT chính thức và evidence asset thật có thể bắt đầu dưới dạng structured placeholder, nhưng bản final không được xem placeholder là asset/evidence thật.
...
Ngày 2026-06-15, đã chốt `docs/graduation-thesis-resources/thesis-defense-deck-methodology-plan.md` làm handoff triển khai cho Phase 8C defense deck. Hướng hiện tại là deck sinh bằng mã nguồn thành PPTX bản chính, export PDF dự phòng, **30 slide chính** cho 20–25 phút trình bày, demo tách riêng khoảng **5–7 phút** theo một golden flow `QR → Cart → Order → KDS → Payment`. Visual direction đã được người viết chọn là **A — Product Dark**, lấy cảm hứng từ landing page Management App: nền zinc tối, cyan/emerald, grid/glow/card, motif QR và dòng chảy dữ liệu, Plus Jakarta Sans + IBM Plex Mono. Logo Trường Đại học Công nghệ Thông tin sẽ được đặt qua slide master sau khi có asset chính thức; nếu chưa có lúc triển khai thì dùng structured placeholder `GLOBAL_SCHOOL_LOGO`, không tự tạo logo giả. Asset chưa có được quản lý bằng structured placeholder/asset registry trong source deck: mỗi placeholder phải nêu ID, loại, luận điểm cần chứng minh, nội dung bắt buộc, nguồn dự kiến, tỉ lệ khung và trạng thái `placeholder/ready/verified`; khi có file thật thì cập nhật path rồi generate lại, không thay ảnh thủ công trong PPTX như workflow chính. Mạch mới dùng cấu trúc `thách thức → bất biến → quyết định thiết kế → logic hoạt động → hiện thực → bằng chứng`, tập trung vào thiết kế hệ thống và giải thích cơ chế đúng lúc thay vì nghiên cứu hóa cách diễn đạt. Defense deck không đưa SaaS onboarding vào phần chính hoặc appendix và chỉ đào sâu Order Confirm Saga; việc report mô tả hai Saga đại diện vẫn được giữ như phạm vi tài liệu. Plan đã ghi rõ slide không đi tuần tự theo Chương 1-7: plan defense deck quyết định narrative; code/tests/runtime config xác minh implementation; canonical technical docs giải thích boundary/contract; report chỉ cung cấp nội dung học thuật, citation, phạm vi chính thức và visual asset khi cần. Session mới có thể bắt đầu triển khai sau khi tạo implementation plan riêng; deck artifact chưa được tạo trong phiên này, chưa sửa slide artifact cũ, chưa thêm citation và chưa sửa `references.bib`.
...
1. **Phase 8C defense deck:** chuyển sang session triển khai từ `thesis-defense-deck-methodology-plan.md`. Agent mới cần tạo implementation plan riêng rồi dựng source deck sinh PPTX/PDF: 30 slide chính, demo riêng 5–7 phút theo golden flow, có appendix, visual direction `A — Product Dark`, không SaaS onboarding, chỉ đào sâu Order Confirm Saga.
2. **Reconcile claim/evidence trước khi freeze các slide bằng chứng:** đồng bộ Chương 6 và traceability từ snapshot `36 covered / 11 partial / 1 gap / 4 extension` sang canonical hiện tại `38 covered / 9 partial / 1 implementation gap / 4 deferred`, đồng thời kiểm tra lại permission count và Phase 7 wording.
...
5. **Phase 7C và Phase 8A/8B:** polish Abstract/phụ lục, build/format/citation audit toàn PDF, rồi reader/reviewer audit về mạch lập luận và overclaim.
...
- Demo deck đã chốt một golden flow riêng khoảng 5–7 phút; khi triển khai cần quyết định asset cụ thể nào được capture thật và asset nào tạm placeholder.
- Trước khi freeze số liệu các slide bằng chứng, cần reconcile snapshot Chương 6 với canonical traceability hiện tại: report đang dùng `36/11/1/4`, phase/test handoff mới dùng `38/9/1/4`.
...
- Không dùng deck 22 slide hoặc brief 30 slide hiện tại như bản final: các artifact này còn claim `Phase 2A implemented`, roadmap cũ và phạm vi service cũ.
