...
> Cập nhật gần nhất: 2026-06-16 (đã dựng lại Phase 8C theo Manus-like Academic Dark: PPTX chính, PDF dự phòng, 30 slide chính và 10 slide appendix).
...
| Nội dung chương             | Chương 2-7 đã có bản nháp nội dung được verify gần nhất; Chương 1/Abstract/phụ lục còn lại | `thesis-report/chapters/02-co-so-ly-thuyet-va-cong-trinh-lien-quan.tex`, `03-phan-tich-yeu-cau.tex`, `04-thiet-ke-va-kien-truc-he-thong.tex`, `05-trien-khai-he-thong.tex`, `06-danh-gia.tex` và `07-ket-luan-va-huong-phat-trien.tex` đang có nội dung trong worktree. Chương 2 đã có Bảng 2.1-Bảng 2.6 và citation IEEE render đúng số; Chương 3 đã có Hình 3.1/Hình 3.2 và Bảng 3.1-Bảng 3.4 được verify; Chương 4 có Hình 4.1-Hình 4.16 (Hình 4.5-4.9 là schema DB per-service, Hình 4.14 là KDS Redis data structures), Bảng 4.1-Bảng 4.11; Phụ lục E không còn lặp schema. Chương 5 đã refactor theo Plan A, có Hình 5.1-Hình 5.5, Bảng 5.1 và Bảng 5.2 về hai Saga đại diện. Chương 6-7 đã audit/rerun theo Plan B và đã làm sạch các câu kiểu workflow/checklist trong phiên 2026-06-05.                                                                                                                                            |
...
| Phase 8C | Defense deck                               | Deck bảo vệ 20–25 phút, speaker notes, appendix phản biện, bản PDF dự phòng và demo fallback                                         | Đã tạo source deck, PPTX, PDF, asset registry và QA render tại `docs/presentations/qrtable-defense-deck/`; còn thay asset thật |
...
Bước tiếp theo khuyến nghị sau khi đã triển khai Plan A, audit/rerun Plan B, triển khai Plan C, hoàn tất plan làm sâu Kafka/Redis, hoàn tất plan security/auth/RBAC Chương 4, làm rõ hai Saga đại diện ở Chương 4-6, draft xong Chương 1 và hoàn tất Plan 2/Plan 2B scope-tone refactor Chương 3/5/7:

1. **Phase 8C asset replacement:** thay các `user-replacement` asset trong `asset-registry.json`, đặc biệt logo trường, sơ đồ kiến trúc tổng thể, service/data ownership, Order Confirm Saga diagram, golden flow screenshots, DB/state evidence, log/trace evidence và test output evidence. Sau mỗi lượt thay asset, cập nhật path trong source deck rồi generate lại PPTX/PDF; không chỉnh thủ công trực tiếp trong PPTX nếu muốn giữ source of truth.
2. **Phase 8C final deck QA:** sau khi thay asset thật, rerun pipeline sinh PPTX/PDF/render PNG, kiểm tra lại font, contrast, tràn chữ, slide count, notes count và nội dung claim. Nếu thêm screenshot/log thật, kiểm tra đã che/anonymize tenant id, token, secret, URL nhạy cảm.
3. **Phase 5D+ (manual):** thay 63 placeholder bằng screenshot demo thật (`chapter-05-ui-gallery-scaffold-plan.md` §8); `\screenshotplaceholderfalse`; backlog `captured`/`verified`. Scaffold đã xong: 14 hình Chương 5, 49 hình Phụ lục A.
4. **Phase 7 deployment/pilot evidence:** hoàn tất public provisioning/HTTPS/external smoke/backup-rollback proof, rồi cập nhật Chương 5-6, appendix và slide bằng URL/domain, health, log rút gọn và webhook callback thật.
5. **Phase 7C và Phase 8A/8B:** đọc lại Lời cảm ơn/Tóm tắt sau phản hồi cá nhân, polish phụ lục, build/format/citation audit toàn PDF, rồi reader/reviewer audit về mạch lập luận và overclaim.
6. **Mini-polish Chương 6 nếu cần:** chỉ mở nếu muốn đồng bộ thêm thuật ngữ `ranh giới đánh giá` hoặc giảm cụm `ở mức thiết kế` trong Chương 6; không coi đây là blocker vì Chương 6 vẫn là nơi hợp lệ để giữ claim policy và future-work table.
...
- Deck đã dùng snapshot canonical `38/9/1/4` từ `docs/phases/phase-5-7-finalization.md`; report Chương 6 vẫn cần reconcile nếu muốn đồng nhất số liệu giữa report và slide.
- Full staff management lifecycle ở technical Phase 4C vẫn là TODO; deck nên claim RBAC, profile/role foundation và permission enforcement, không claim hoàn tất toàn bộ create/assign/disable staff workflow.
...
- Khi chuẩn bị artifact Saga cho bản nộp, terminal output unit/contract đã có ở Phụ lục D; cần quyết định cách thu D-SAGA-3 đến D-SAGA-6 gồm integration opt-in, snapshot DB/outbox và screenshot UI happy path.
- Technical Phase 6/observability đã được quyết định là không đưa vào nội dung khóa luận; không mở mục lý thuyết, không đưa vào scope/out-of-scope/hướng phát triển, không thêm citation riêng cho mảng này.
- Phase 7 đã có production Compose, bootstrap, monitoring và reverse-proxy foundation; phần còn lại cần thu thập là public URL/domain, HTTPS certificate, external smoke luồng lõi, backup/rollback proof, log rút gọn, webhook callback và release/config checklist.
...
- Technical Phase 6 Observability đã có dấu vết code/config trong repo hiện tại nhưng không đưa vào nội dung khóa luận. Plan 2 đã loại Technical Phase 6/observability khỏi Chương 3/5/7; Chương 6 vẫn giữ claim policy hiện tại và chỉ nên sửa bằng mini-plan riêng nếu muốn đồng bộ thuật ngữ sâu hơn.
- Technical Phase 7 Production Deployment đã có foundation trong code/config nhưng chưa có đủ public deployment evidence. Không dùng local dev screenshot hoặc foundation artifact để claim production-ready; chỉ nâng mức kết luận khi có public HTTPS, external smoke, backup/rollback và webhook evidence thật.
