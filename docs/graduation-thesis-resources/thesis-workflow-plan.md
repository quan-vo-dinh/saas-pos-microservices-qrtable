...
> Cập nhật gần nhất: 2026-06-05 (thực thi Plan 2B semantic tone audit Chương 3/5/7 và build XeLaTeX pass).
...
1. **Phase 5D+ (manual):** thay 63 placeholder bằng screenshot demo thật (`chapter-05-ui-gallery-scaffold-plan.md` §8); `\screenshotplaceholderfalse`; backlog `captured`/`verified`. Scaffold đã xong: 14 hình Chương 5, 49 hình Phụ lục A.
2. **Phase 7 deployment/pilot evidence:** triển khai hệ thống thật như nhánh chắc chắn của khóa luận, rồi cập nhật Chương 5-6 và phụ lục bằng URL/domain, HTTPS, kiểm tra sức khỏe, kiểm thử khói, log rút gọn và webhook callback.
3. **Phase 7C còn lại:** polish Abstract, tóm tắt và phụ lục cần thiết.
4. **Phase 8A/8B:** build/format/citation audit toàn PDF, rồi reader/reviewer audit về mạch lập luận và overclaim.
5. **Mini-polish Chương 6 nếu cần:** chỉ mở nếu muốn đồng bộ thêm thuật ngữ `ranh giới đánh giá` hoặc giảm cụm `ở mức thiết kế` trong Chương 6; không coi đây là blocker vì Chương 6 vẫn là nơi hợp lệ để giữ claim policy và future-work table.
...
- Phase 7 sẽ triển khai production/pilot trong vài ngày tới như nhánh chắc chắn; cần thu thập URL/domain, HTTPS, kiểm tra sức khỏe, kiểm thử khói luồng lõi, log rút gọn, webhook callback và release/config checklist để cập nhật khung Chương 5-6 sau khi deploy.
...
- Technical Phase 7 Production Deployment có khung ngắn trong Chương 5-6 vì chắc chắn triển khai trong vài ngày tới. Không dùng local dev screenshot hoặc ý định deploy để claim đã kiểm chứng; chỉ đổi sang kết quả đã đạt khi có artifact production/pilot thật.
...
- Hai flow Saga đã có diagram: Hình 5.2 cho Order Confirm Saga và Hình 5.5 cho SaaS Onboarding Mini-Saga. Không tạo thêm diagram mới nếu chỉ cần trả lời câu hỏi hiện tại; nếu polish thẩm mỹ sau này thì sửa `.mmd`, render lại PDF và giữ đúng nhãn/caption đã cập nhật.
