#!/usr/bin/env bash
# Render Chapter 4 academic architecture diagrams from PlantUML source to SVG, PDF, and PNG.
set -euo pipefail
...
if [[ $# -gt 0 ]]; then
  ...
  targets=()
  for src in assets/diagrams/chapter4-*.puml; do
    [[ -f "$src" ]] || continue
...
echo "OK: Chapter 4 PlantUML diagrams rendered."
