...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.31\textwidth}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.16\textwidth}}
  ...
  D -- Giới hạn hoặc hướng phát triển & Chưa có đủ môi trường, dữ liệu, nhà cung cấp thực tế hoặc số đo định lượng. & Chưa hình thành kết luận kiểm chứng trong phạm vi hiện tại. & Kiểm thử tải, kiểm chứng dài hạn, khả năng quan sát. \\
\end{longtable}
}
...
Một số kết quả vẫn cần đọc với phạm vi phù hợp. Kiểm thử hiện tại đủ để đánh giá các luồng chính và một số nhánh lỗi đại diện, nhưng chưa thay thế kiểm thử toàn bộ bề mặt API, kiểm thử tải hoặc kiểm chứng với nhà cung cấp thanh toán thật.
...
\subsection{Phân tích kết quả kiểm thử trên Allure Report}
...
\subsection{Các kịch bản kiểm chứng ràng buộc của Order Confirm Saga}
...
Phạm vi hiện tại chưa có số liệu định lượng về độ trễ, thông lượng, tỉ lệ lỗi hoặc mức sử dụng tài nguyên. Vì vậy, chương này không so sánh QRTable với kiến trúc nguyên khối về hiệu năng và không kết luận hệ thống đã chịu tải lớn. Một đánh giá hiệu năng đầy đủ cần môi trường đo ổn định, dữ liệu kiểm thử rõ ràng và phương pháp đo lặp lại được.

Những gì có thể kết luận là thiết kế tạo cơ sở hợp lý cho mở rộng từng phần. Các thành phần có thể được tách theo đặc điểm tải, bộ nhớ đệm hỗ trợ các trạng thái cần truy cập nhanh, và sự kiện bất đồng bộ giúp giảm phụ thuộc trực tiếp giữa một số bước xử lý. Tuy nhiên, các nhận định này vẫn ở mức cơ sở kiến trúc, chưa phải kết quả đo tải.

Các số đo hiệu năng vì vậy được xem là hướng phát triển tiếp theo, thay vì kết quả đã hoàn tất trong phạm vi khóa luận.
...
\subsection{Phân tích kết quả đóng gói và giao diện triển khai thực tế}
...
{\scriptsize\sloppy
\setlength{\tabcolsep}{3pt}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}}
  ...
  \bottomrule
  \endfoot
  Chưa có kiểm thử tải định lượng. & Không kết luận thông lượng, độ trễ hoặc sức chứa dưới tải lớn. & Đánh giá khả năng mở rộng ở mức cơ sở kiến trúc và không tạo số liệu. & Bổ sung kịch bản đo tải cho các luồng nghiệp vụ chính. \\
  Cô lập dữ liệu theo đơn vị thuê bao chưa phủ toàn bộ bề mặt khi vận hành. & Kết luận còn gắn với thiết kế và kiểm thử đại diện. & Không viết như đã kiểm chứng toàn bộ mọi trường hợp. & Chuẩn hóa dữ liệu nhiều đơn vị thuê bao và mở rộng kiểm thử hồi quy. \\
  Phân quyền phụ thuộc dữ liệu khởi tạo. & Kết quả có thể thay đổi nếu dữ liệu vai trò không ổn định. & Chỉ dùng ảnh hoặc kết quả khi dữ liệu kiểm thử được chuẩn hóa. & Cố định dữ liệu vai trò và quy trình đăng nhập trong môi trường kiểm thử. \\
  Saga chưa đạt mức củng cố vận hành đầy đủ. & Không kết luận đã bảo đảm tuyệt đối cho mọi lỗi phân tán. & Chỉ nói hai Saga đại diện đã có kiểm thử và kết quả đối chiếu tương ứng. & Bổ sung cơ chế thử lại, quan sát lỗi và kiểm chứng dài hạn. \\
  Chưa triển khai công khai toàn bộ ngăn xếp microservices. & Không đánh giá tính sẵn sàng cao hoặc khả năng vận hành dài hạn. & Chỉ dùng Docker/Vercel như kết quả đóng gói hoặc triển khai một phần. & Hoàn thiện môi trường triển khai đầy đủ và quy trình kiểm chứng sau triển khai. \\
  Khả năng quan sát chưa có dữ liệu vận hành đầy đủ. & Không kết luận hệ thống đã đạt mức quan sát sản xuất. & Không đưa dashboard quan sát nếu chưa có dữ liệu vận hành thực tế. & Bổ sung log, metric, trace và cảnh báo trong môi trường vận hành. \\
  Kiểm chứng SePay với nhà cung cấp thực tế chưa thuộc đường chạy mặc định. & Không kết luận kiểm chứng nhà cung cấp đã được tự động hóa. & Dùng mô phỏng và kiểm thử hợp đồng cho phạm vi hiện tại. & Kiểm chứng thủ công với giao dịch thực tế khi có điều kiện phù hợp. \\
  Chưa có hàng đợi thao tác ngoại tuyến đầy đủ. & Không kết luận ứng dụng hỗ trợ đầy đủ thao tác ghi khi mất mạng. & Chỉ đánh giá kết nối lại, tải lại và trạng thái mới nhất. & Thiết kế cơ chế đồng bộ nền và xử lý xung đột. \\
\end{longtable}
}
