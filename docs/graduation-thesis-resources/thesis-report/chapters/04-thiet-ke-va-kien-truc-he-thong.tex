...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.31\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}}
  ...
  Next.js và React & Ứng dụng quản trị & Phù hợp với client quản trị POS/KDS/báo cáo, nhiều trạng thái dữ liệu và luồng xác thực nhân viên. & Không biến client quản trị thành nơi xử lý nghiệp vụ; nghiệp vụ vẫn ở dịch vụ sở hữu. \\
  ...
  Prometheus, Grafana, Tempo\slash OpenTelemetry và Loki & Lớp quan sát hệ thống & Thu thập metric, hiển thị dashboard, truy vết request đại diện và chuẩn bị khả năng đối chiếu log khi có \texttt{traceId}. & Minh chứng hiện tại chủ yếu ở môi trường local; Loki không thay thế log nghiệp vụ nếu log chưa đủ dữ liệu đối chiếu. \\
  k6 & Công cụ đánh giá hiệu năng đại diện & Tạo tải HTTP có kiểm soát cho các luồng đọc, đặt món và xác nhận đơn/KDS để bổ sung số liệu định lượng ở Chương 6. & Không phải công nghệ chạy hệ thống của QRTable; không dùng kết quả local để suy rộng thành stress test sản xuất. \\
...
Hình~\ref{fig:chapter4-technology-integration-map} đặt các công nghệ chạy hệ thống vào năm lớp kiến trúc: ứng dụng người dùng, lớp biên, dịch vụ miền nghiệp vụ, hạ tầng dữ liệu và sự kiện, cùng nhà cung cấp ngoài. Các luồng chính thể hiện ứng dụng truy cập qua BFF, dịch vụ giao tiếp nội bộ bằng TCP/gRPC, Kafka truyền một số sự kiện miền, Redis giữ bộ nhớ đệm hoặc trạng thái khi vận hành, và webhook thanh toán được BFF định tuyến đến Payment hoặc SaaS. Lớp quan sát và công cụ benchmark được trình bày trong bảng công nghệ vì chúng phục vụ vận hành và đánh giá, nhưng không làm thay đổi ranh giới nghiệp vụ của hình.
...
Ở lớp dịch vụ miền nghiệp vụ, hệ thống gồm Authorizer, User-Access, SaaS, Catalog, Order, Kitchen và Payment. Mỗi dịch vụ đại diện cho một ngữ cảnh giới hạn: xác minh danh tính, hồ sơ ứng dụng/RBAC, vòng đời đơn vị thuê bao, thực đơn/bàn/tồn kho, đơn/phiên/hóa đơn, hàng đợi KDS và quyết toán thanh toán. Module \texttt{product} trong monorepo chỉ là phần mẫu do Nx sinh ra, không thuộc kiến trúc chính của QRTable vì không gánh trách nhiệm nghiệp vụ của hệ thống.

Ở lớp hạ tầng, QRTable dùng PostgreSQL cho dữ liệu nghiệp vụ quan hệ, MongoDB cho User-Access, Redis cho bộ nhớ đệm/phiên/hàng đợi KDS/cờ trạng thái chạy, Kafka cho sự kiện miền nghiệp vụ, Keycloak cho định danh nhân viên, SePay\slash VietQR cho thanh toán chuyển khoản, Cloudinary cho ảnh thực đơn và Docker\slash Docker Compose cho môi trường chạy. Lớp quan sát dùng Prometheus, Grafana, Tempo/OpenTelemetry và Loki để thu thập metric, trình bày dashboard, truy vết request và chuẩn bị đối chiếu log; còn k6 được dùng ở Chương 6 như công cụ tạo tải đại diện cho đánh giá định lượng.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.28\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}}
  ...
  TCP RPC & Payment -- Order & Lấy trạng thái hóa đơn mới nhất và đánh dấu đã thanh toán theo cách có idempotency cho đường nhanh/phục hồi. & Payment không sở hữu hóa đơn/phiên. \\
...
Kafka được sử dụng khi thay đổi trạng thái cần kích hoạt quy tắc ở ngữ cảnh giới hạn khác, khi bên phát không phải chờ bên tiêu thụ, hoặc khi sự kiện được tạo bởi tiến trình nền. Sự kiện phát sinh từ giao dịch CSDL đi qua outbox giao dịch để giảm rủi ro ghi kép. Các tác dụng phụ chỉ nhằm thông báo cho ứng dụng hoặc làm mất hiệu lực bộ nhớ đệm sau phản hồi TCP được xử lý trực tiếp qua BFF.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.2\textwidth}>{\raggedright\arraybackslash}p{0.36\textwidth}>{\raggedright\arraybackslash}p{0.36\textwidth}}
  ...
  \texttt{aggregate\_id} & Định danh aggregate phát sinh sự kiện. & Hỗ trợ kiểm toán, idempotency và đối chiếu từ sự kiện về đơn/thanh toán/đơn vị thuê bao. \\
...
Bảng~\ref{tab:chapter4-kafka-topic-registry} xác định phạm vi năm topic Kafka của kiến trúc hiện tại. Các thay đổi như cập nhật giỏ, yêu cầu hóa đơn, chuyển bàn, thay đổi hàng đợi KDS, tạm ngưng đơn vị thuê bao hoặc cập nhật thực đơn được truyền bằng sự kiện socket, gợi ý nội bộ Redis hoặc cơ chế làm mất hiệu lực bộ nhớ đệm tùy theo yêu cầu nghiệp vụ.
...
Hình~\ref{fig:chapter4-redis-ownership} tách Redis theo nhóm khóa và dịch vụ sở hữu. Cách trình bày này giúp tránh hiểu nhầm Redis là kho trạng thái chung của toàn hệ thống. Với mỗi nhóm khóa, Redis chỉ giữ bộ nhớ đệm, cờ vận hành ngắn hạn hoặc bản chiếu khi vận hành; nguồn dữ liệu có thẩm quyền vẫn nằm ở dịch vụ sở hữu.
...
QRTable sử dụng cơ chế xác thực theo từng nhóm tác nhân. Khách tại bàn truy cập qua mã QR và phiên; nhân viên vận hành dùng quyền theo vai trò trong một đơn vị thuê bao; chủ quán và quản lý có thêm quyền quản trị nội bộ; quản trị viên nền tảng có quyền xuyên đơn vị thuê bao; SePay là hệ thống ngoài gửi đường gọi lại. Bảng~\ref{tab:chapter4-auth-actor-model} tóm tắt mô hình này.
...
Chuỗi guard ở BFF giúp tách các câu hỏi bảo mật thành nhiều lớp. UserGuard hoặc SessionGuard trả lời ``người gọi là ai'', TenantGuard trả lời ``yêu cầu thuộc đơn vị thuê bao nào'', PermissionGuard trả lời ``tác nhân có quyền thực hiện hành động này không'', còn PlanFeatureGuard trả lời ``đơn vị thuê bao có được dùng tính năng này theo gói không''. Bảng~\ref{tab:chapter4-security-control-layers} diễn đạt các lớp này như ngữ cảnh yêu cầu và câu hỏi kiến trúc, thay vì phân tích từng phương thức của framework.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.2\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}}
  ...
  \midrule
  \endhead
  Xác định tác nhân & Người gọi là nhân viên, quản trị viên hay khách theo phiên? & HTTP/WebSocket từ ứng dụng & Có định danh hoặc ngữ cảnh phiên trước khi xử lý. \\
  Khóa đơn vị thuê bao & Yêu cầu đang thuộc \texttt{tenantId} nào? & Hầu hết API có phạm vi đơn vị thuê bao & Không để tác nhân của đơn vị thuê bao A truy cập dữ liệu của đơn vị thuê bao B. \\
  Kiểm tra quyền thao tác & Tác nhân có quyền thực hiện hành động không? & API nhân viên, chủ quán và quản trị & Ngăn vai trò không phù hợp thao tác thực đơn, đơn, thanh toán hoặc báo cáo. \\
  Kiểm tra quyền theo gói & Gói thuê bao có bao gồm tính năng không? & Bảng điều khiển, báo cáo và tuyến khóa theo tính năng & Phân biệt quyền thương mại theo gói với RBAC của người dùng. \\
  Kiểm tra phiên/bàn & Phiên có khớp bàn, đơn vị thuê bao và trạng thái hợp lệ không? & PWA khách & Giữ khách trong phạm vi phiên hiện tại. \\
  Định tuyến đường gọi lại ngoài & Webhook hoặc OAuth có đúng tuyến, bí mật/header, mã tham chiếu hoặc trạng thái không? & Webhook và OAuth SePay & Chuyển đúng Payment hoặc SaaS, hỗ trợ xử lý lặp an toàn và kiểm toán. \\
  \bottomrule
\end{longtable}
}
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.2\textwidth}}
  ...
  \midrule
  \endhead
  RBAC & Người dùng có quyền thực hiện hành động này không? & Nhân viên phục vụ xác nhận đơn; nhân viên bếp/bar cập nhật KDS; chủ quán quản lý thực đơn và nhân sự. & Không tự xác định đơn vị thuê bao hoặc gói dịch vụ. \\
  Cô lập dữ liệu theo đơn vị thuê bao & Dữ liệu/hành động thuộc đơn vị thuê bao nào? & API, DB query, Redis key, Kafka payload và WebSocket room đều mang ngữ cảnh \texttt{tenantId}. & Không phải cơ sở dữ liệu riêng cho từng đơn vị thuê bao trong thiết kế hiện tại. \\
  Quyền theo gói & Đơn vị thuê bao có được dùng tính năng này theo gói không? & Chủ quán và quản lý có \texttt{report.read\_own}, còn báo cáo phụ thuộc \texttt{analytics\_basic}. & Không phải vai trò người dùng. \\
  Quyền nền tảng & Quản trị viên có quyền nền tảng nào? & Khởi tạo đơn vị thuê bao, quản lý gói, xem phân tích nền tảng và xem chi tiết đơn vị thuê bao có kiểm soát. & Vẫn chịu kiểm soát truy cập và kiểm toán. \\
  \bottomrule
\end{longtable}
}
...
Thiết kế bảo mật xác lập hai luồng xác thực, chuỗi guard cho nhân viên và khách, ma trận quyền cho sáu vai trò nội bộ, tác nhân khách theo phiên, quyền theo gói cho báo cáo và hợp đồng webhook theo tuyến. Phạm vi hiện tại chưa bao gồm kiểm thử xâm nhập chính thức, tự động xoay vòng bí mật, mTLS nội bộ, service mesh, WAF hoặc kiểm thử toàn bộ bề mặt API liên đơn vị thuê bao. Kết quả kiểm thử các cơ chế đã triển khai được trình bày ở Chương~\ref{chap:danh-gia}.
...
Thiết kế triển khai tạo nền tảng cho trình diễn và thu thập minh chứng trên môi trường công khai. Tính sẵn sàng cao, khả năng chịu tải, kiểm thử gây lỗi có chủ đích, thời gian hoạt động và khả năng phục hồi cần môi trường cùng quy trình kiểm chứng riêng nên không thuộc kết quả của mục này.

\section{Tổng hợp đánh đổi kiến trúc}

Việc sử dụng vi dịch vụ thay vì một khối ứng dụng duy nhất giúp QRTable tách rõ các ngữ cảnh giới hạn có nhịp thay đổi khác nhau: Catalog/tồn kho, Order/phiên/hóa đơn, Kitchen/KDS, Payment/quyết toán, SaaS/gói thuê bao và User-Access/RBAC. Lợi ích chính là trách nhiệm dữ liệu rõ hơn, giảm phụ thuộc chéo miền nghiệp vụ và dễ trình bày/đánh giá từng luồng theo ranh giới dịch vụ. Đổi lại, hệ thống phải xử lý độ phức tạp phân tán: lỗi mạng, nhất quán cuối cùng (eventual consistency), idempotency, khả năng theo dõi và lệch contract. Vì vậy, vi dịch vụ trong QRTable được dùng như một lựa chọn thiết kế có đánh đổi, không phải khẳng định luôn tốt hơn kiến trúc nguyên khối.

Việc kết hợp RPC đồng bộ và Kafka cũng là một đánh đổi. TCP/gRPC đơn giản hơn cho lệnh/truy vấn cần phản hồi tức thời và lỗi rõ ràng với người dùng. Kafka phù hợp hơn cho sự kiện miền bất đồng bộ, tách thời gian và phân phối. Nếu dùng Kafka cho mọi yêu cầu, hệ thống sẽ khó gỡ lỗi hơn và tăng độ trễ cho luồng khách/nhân viên. Nếu chỉ dùng RPC, các tác dụng phụ như phiếu KDS, hoàn tất thanh toán hoặc cảnh báo SLA sẽ dễ bị siết chặt với dịch vụ phát sự kiện. Quy trình lựa chọn ở Hình~\ref{fig:chapter4-kafka-decision-flow} là cách QRTable giữ cân bằng giữa hai hướng này.
