...
Thứ sáu, các thao tác ghi ảnh hưởng đến trạng thái bên ngoài phải hướng tới idempotency (tính lũy đẳng), bù trừ có kiểm soát và dấu thời gian phía máy chủ. Điều này đặc biệt quan trọng ở webhook thanh toán, xác nhận đơn, xuất bản outbox và gửi đơn của khách. Trong phạm vi hiện tại, khóa luận chỉ xem saga pattern như thiết kế đại diện ở hai luồng: xác nhận đơn và khởi tạo đơn vị thuê bao (onboarding). Các tăng cường sâu hơn như trạng thái saga bền vững, tiến trình nền retry đầy đủ, CDC hoặc bảo đảm phân phối exactly-once được xem là hướng phát triển, không phải kết quả đã chứng minh ở mức vận hành thực tế.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.31\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}}
  ...
  SePay\slash VietQR và Cloudinary & Thanh toán chuyển khoản và ảnh thực đơn & SePay\slash VietQR phục vụ thanh toán hóa đơn/gói thuê bao; Cloudinary phục vụ tệp ảnh thực đơn. & Chỉ khẳng định mức kiểm chứng nhà cung cấp hoặc vận hành thực tế khi có bằng chứng tương ứng. \\
\end{longtable}
}
...
Ở lớp hạ tầng, QRTable dùng PostgreSQL cho dữ liệu nghiệp vụ quan hệ, MongoDB cho User-Access, Redis cho bộ nhớ đệm/phiên/hàng đợi KDS/cờ trạng thái chạy, Kafka cho sự kiện miền nghiệp vụ, Keycloak cho định danh nhân viên, SePay\slash VietQR cho thanh toán chuyển khoản, Cloudinary cho ảnh thực đơn và Docker\slash Docker Compose cho môi trường chạy. Grafana, Loki, Prometheus và Tempo được trình bày như hướng thiết kế quan sát hệ thống; các khẳng định về vận hành đầy đủ chỉ nên nêu khi có bằng chứng chạy thật.
...
Bảng~\ref{tab:chapter4-kafka-topic-registry} liệt kê năm topic được chấp nhận trong sổ đăng ký hiện tại. Mỗi dòng trình bày topic như một hợp đồng (contract) kiến trúc: bên phát, bên tiêu thụ, khóa phân vùng, payload chính và cơ chế độ tin cậy/idempotency ở mức thiết kế. Payload trong bảng chỉ nêu các trường đại diện để người đọc hiểu ý nghĩa sự kiện, không thay thế định nghĩa DTO đầy đủ trong triển khai.
...
Ma trận permission đầy đủ của QRTable nằm ở tài liệu kiến trúc RBAC, không được lặp toàn bộ trong chương chính. Ở mức thiết kế, điểm cần nhấn mạnh là Customer không nằm trong \texttt{role.json}; khách được bảo vệ bằng session/ownership. Ngược lại, staff/owner/manager/super admin dùng role/permission do Keycloak và User-Access phối hợp cung cấp cho BFF.
...
Luồng VietQR cũng phải giữ nguyên tắc idempotency. Một webhook từ nhà cung cấp có thể đến trễ, đến nhiều lần hoặc thiếu trường nhận dạng trong một số tình huống. Payment/SaaS cần khớp tham chiếu trong \texttt{code}, \texttt{content} hoặc dữ liệu liên quan, kiểm tra số tiền, ghi kiểm toán và chỉ chuyển trạng thái khi đủ điều kiện. Phần này mô tả thiết kế tích hợp và ranh giới kiểm soát; mức độ kiểm chứng nhà cung cấp thật, webhook giả lập hoặc kiểm thử thủ công sẽ thuộc Chương 6.

\section{Thiết kế triển khai, quan sát và giới hạn khẳng định}

QRTable có thiết kế chạy bằng Docker/Docker Compose cùng các thành phần phụ thuộc như PostgreSQL, MongoDB, Redis, Kafka và Keycloak. Nhóm công cụ quan sát gồm Grafana, Loki, Prometheus và Tempo. Thiết kế này giúp gom môi trường phát triển/trình diễn vào một bộ môi trường có thể tái lập, giảm chênh lệch giữa máy phát triển và môi trường trình diễn.

Tuy nhiên, triển khai và quan sát vận hành là khu vực cần diễn đạt thận trọng. Có cấu trúc Docker và bộ công cụ giám sát trong thiết kế không đồng nghĩa hệ thống đã đạt sẵn sàng cao, quan sát sẵn sàng vận hành thực tế, kiểm thử áp lực dưới tải lớn hoặc kiểm thử gây lỗi có chủ đích. Trong phạm vi Chương 4, triển khai/quan sát được trình bày như hướng thiết kế vận hành và nền tảng cho trình diễn/đánh giá. Các khẳng định về bảng điều khiển vận hành thật, độ phủ truy vết/nhật ký, thời gian hoạt động, thông lượng hoặc khả năng phục hồi chỉ nên đưa vào Chương 6 nếu có minh chứng kiểm chứng.
