...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{chapter2-monolith-vs-microservices.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{chapter2-kafka-event-flow.pdf}
...
\begin{itemize}
  \item Giao dịch nghiệp vụ nội bộ được hoàn tất tại dịch vụ sở hữu dữ liệu trước khi sự kiện được phát qua Kafka.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{chapter2-outbox-saga-overview.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{chapter2-websocket-hint-refetch.pdf}
...
Một điểm khác là thời gian thực không thay thế hàng đợi hoặc trình môi giới. WebSocket phù hợp để đẩy thông báo tới ứng dụng đang kết nối. Kafka phù hợp hơn cho sự kiện miền nghiệp vụ bền vững giữa các dịch vụ backend. Redis có thể phù hợp cho hàng đợi khi vận hành hoặc pub/sub nội bộ. Việc chọn sai công cụ có thể làm hệ thống khó vận hành: dùng WebSocket để lưu sự kiện bền vững sẽ mất dữ liệu khi ứng dụng ngoại tuyến; dùng Kafka chỉ để báo giao diện làm mới sau một phản hồi HTTP có thể làm hệ thống nặng hơn cần thiết.

\section{Xác thực, phân quyền và bảo mật trong SaaS POS}

Xác thực trả lời câu hỏi người gọi là ai; phân quyền trả lời người đó có được phép thực hiện hành động hay không. Trong SaaS POS, hai câu hỏi này phải luôn đi cùng ngữ cảnh đơn vị thuê bao. Chủ quán có quyền trong đơn vị thuê bao của mình không có nghĩa người đó có quyền xem dữ liệu của đơn vị thuê bao khác. Nhân viên có quyền xác nhận đơn không nhất thiết có quyền cập nhật cấu hình thanh toán hoặc gói thuê bao.
...
Bảng~\ref{tab:chapter2-related-systems} tổng hợp khía cạnh liên quan của các hệ thống và giới hạn của nguồn công khai. Do không có dữ liệu đo lường hoặc thông tin kiến trúc nội bộ của sản phẩm thương mại, bảng tập trung vào giá trị tham khảo cho QRTable thay vì xếp hạng các hệ thống.
...
\begin{longtable}{>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.31\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.15\textwidth}}
  ...
  \addlinespace
  WebSocket / thời gian thực & Giao diện nhận gợi ý nhanh nhưng trạng thái backend vẫn là nguồn đáng tin. & Ứng dụng tải lại trạng thái từ API sau tín hiệu thời gian thực. & Chương 4, 5, 6 \\
  \addlinespace
  ...
  \addlinespace
  Hiệu năng và khả năng quan sát & Hệ thống phân tán cần số liệu độ trễ, thông lượng, lỗi và trace để đánh giá có kiểm soát. & k6 được dùng cho tải đại diện; Prometheus/Grafana hỗ trợ metric/dashboard; Tempo/OpenTelemetry hỗ trợ trace; Loki chỉ có giá trị khi log ứng dụng đủ dữ liệu đối chiếu. & Chương 3, 6 \\
\end{longtable}
}
