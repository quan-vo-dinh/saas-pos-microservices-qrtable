...
| Authentication/RBAC           | Keycloak/JWT cho staff/admin, session guard cho customer, 6 roles/66 permissions verified                                           | Mạnh                                | Có thể viết là guard chain và permission seed đã được kiểm chứng bằng unit/contract/live smoke script. |
...
| Notification/email            | Phase 4C deferred, không có `apps/notification` runtime                                                                             | Hướng phát triển                    | Không đưa notification/email receipt như kết quả đánh giá.                                             |

## 5. Evaluation claim policy table

| Mức claim                              | Điều kiện evidence                                                                      | Cách diễn đạt được phép                                                          | Ví dụ QRTable                                                                                  |
| -------------------------------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| A - Đã kiểm chứng tự động              | Có unit/contract/integration/E2E pass trong traceability, command hoặc test location rõ | "đã được kiểm chứng bằng unit/contract/integration test"                         | VND rounding, order submit idempotency, KDS dedupe Redis integration, Kafka 6-topic registry.  |
...
| Notification/email service chưa có runtime                                  | `P1-PHASE4C-NOTIFICATIONS` deferred                                                  | Không dùng email/welcome/receipt làm kết quả                                  | Triển khai Notification service hoặc tích hợp SMTP/provider, rồi thêm tests và demo evidence.             |
...
| Traceability            | Phase 5 traceability ghi nhận 52 dòng P0/P1, trong đó 38 covered, 10 partial, 1 implementation gap và 3 deferred-by-phase.                                                |
...
| "Tất cả P0 đã covered"                                                                    | Còn bốn P0 partial.                                                                                      |
...
| "Full offline queue, Notification Service/email, CDC/Debezium/saga hardening đã hoàn tất" | Các mục này đang implementation-gap/deferred/future scope.                                               |
