...
Addendum 2026-06-01: technical Phase 4D Dashboard & Reporting đã được bổ sung sau khi Phase 6A/6B draft xong. Chương 6 cần thêm bằng chứng cho BFF reporting guard specs, `PlanFeatureGuard`, frontend entitlement UI và screenshot/browser evidence nếu có; không được tự nâng claim thành benchmark hoặc full E2E nếu chưa chạy.

Sync 2026-06-04 sau Plan B: Chương 6 đã được refactor thành `Kiểm chứng và đánh giá hệ thống QRTable`, đã backfill dashboard/reporting vào truy vết yêu cầu, kiểm chứng chức năng, entitlement theo gói, NFR/evidence và limitation. Chương 7 cũng đã được viết thành nội dung thật và kết luận theo mức evidence của Chương 6. Demo/screenshot browser và production/pilot artifact thật vẫn chưa được coi là bằng chứng đã có.
...
| Dashboard/reporting      | Owner/Manager, Super Admin                       | Technical Phase 4D docs, BFF reporting specs, `PlanFeatureGuard`, reports frontend code | Đã backfill vào Chương 6: report permissions, plan feature entitlement và UI lock states; browser proof/screenshot còn cần capture nếu muốn claim demo. |
...
| Dashboard/reporting entitlement | `report.read_own`, `report.read_any`, `PlanFeatureGuard`, reports frontend entitlement UI                                           | Mạnh ở code/spec; đã backfill vào Chương 6 | Viết là permission/feature-gated reporting đã có bằng chứng kỹ thuật; chỉ gọi demo/browser-verified khi đã capture/chạy thật. |
...
| Dashboard/reporting còn thiếu artifact demo thật                            | Technical Phase 4D docs/source/spec đã có và Plan B đã backfill vào Chương 6; screenshot/browser output chưa được chốt làm evidence | Chỉ claim permission/entitlement ở mức code/spec; demo UI cần bổ sung         | Capture Owner/Manager dashboard, Super Admin analytics và UI lock state bằng môi trường demo thật.        |
...
Sau sync Plan B ngày 2026-06-04, các bảng gợi ý đã được hiện thực trong Chương 6 theo số hiệu 6.1-6.5 ở trên. Điểm còn lại không phải nội dung Chương 6-7, mà là thu thập artifact thật nếu muốn nâng mức demo/pilot: screenshot browser, URL/domain, HTTPS, health check, smoke test, public webhook callback, log rút gọn và một flow lõi trên môi trường deploy.
